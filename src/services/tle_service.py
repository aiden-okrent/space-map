...
class TLEService:
    ...
    def fetchTLE(self, satnum: str):
        ...
        async def download_tle_data(satnum: str):
            ...
            try:
                ...
                tle_data = await asyncio.get_event_loop().run_in_executor(
                    None, response.read
                )
        ...
        if file:
            ...
            if not tle_data:
                print(
                    "No TLE data found for satnum: " + str(satnum), "in path: " + file
                )
                return None
            else:
                if "No GP data found" in tle_data:
                    os.remove(file)
                    print(
                        "No satellite found in Celestrak database for satnum: "
                        + str(satnum)
                    )
                    return None
                else:
    ...
    def searchDir(self, text: str):
        ...
        if text.isdigit():
            satnums = [
                int(filename.replace(".tle", ""))
                for filename in os.listdir(TLE)
                if text == filename.replace(".tle", "")
            ]
        else:
            satnums = [
                int(filename.replace(".tle", ""))
                for filename in os.listdir(TLE)
                if text.lower() in open(os.path.join(TLE, filename)).read().lower()
            ]
    ...
    def new(self, satnum: int):
        ...
        if not path:
            print(
                "No existing TLE file found for satnum " + str(satnum),
                "downloading new TLE file from Celestrak database.",
            )
        ...
        if not tle_data == None:
            ...
            if not satellite.epochValid_at(datetime.datetime.now()):
                print(
                    "Epoch is invalid for satellite with catalog ID: " + str(satnum),
                    "within a margin of " + str(14),
                    "days. Requesting fresh TLE data.",
                )
                tle_data = self.fetchTLE(satnum)
                satellite = Satellite(
                    parent=self.model.eci,
            ...
            print(
                "Failed to build Satellite object for Catalog ID: " + str(satnum),
                "because the TLE data returned None.",
            )
            return None
